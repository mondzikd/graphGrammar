...
def visualize(graph, level=None, overlap=True):  # TODO: Refactor
    if level is not None and level <= graph.graph["max_level"]:
        ...
        graph_height = max(graph.nodes[0]["pos"][1], graph.nodes[1]["pos"][1], graph.nodes[2]["pos"][1],
                           graph.nodes[3]["pos"][1]) - min(graph.nodes[0]["pos"][1], graph.nodes[1]["pos"][1],
                                                           graph.nodes[2]["pos"][1], graph.nodes[3]["pos"][1])
        color_map = list(map(lambda node_id: graph.nodes[node_id]['color'], graph.nodes))
        label_dict = dict(map(lambda node_id: (node_id, graph.nodes[node_id]['label']), graph.nodes))
        pos = {node_id: move_up(graph.nodes[node_id]["pos"], graph.nodes[node_id]["level"] * graph_height * 2) for
               node_id in graph.nodes}

    if not overlap:
        handle_overlapping(pos)
